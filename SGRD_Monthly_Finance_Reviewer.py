...
\
st.title("\uc0\u55357 \u57059 \u65039  Road District Monthly Audit Tool")\
...
if uploaded_file:\
    ...
    if "Revenue" in sheet_names or "ACTUAL Tax revenue" in sheet_names:\
        ...
        st.subheader("\uc0\u55357 \u56522  Revenue Analysis")\
        \
        ...
        try:\
            ...
            if may_current < (may_avg * 0.5):\
                st.error(f"\uc0\u9888 \u65039  **Anomaly in May Revenue:** Current May revenue ($\{may_current:,.2f\}) is significantly lower than the 3-year average ($\{may_avg:,.2f\}).")\
...
\
    ...
    if "Budget" in sheet_names and "Cash" in sheet_names:\
        st.subheader("\uc0\u55357 \u56496  Cash & Budget Reconciliation")\
        ...
        \
        if reconciled:\
            st.success("\uc0\u9989  Reconciliation Passed: Ending Cash balance matches reported monthly spending.")\
        else:\
            st.error("\uc0\u10060  Reconciliation Failed: There is a discrepancy between the Cash account and the Expense report.")\
\
    # --- LOGIC: BURN RATE ---\
    st.subheader("\uc0\u55357 \u56613  Fiscal Year Burn Rate")\
